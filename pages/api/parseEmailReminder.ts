...
// import { verifyMakeWebhook } from '@/lib/auth' // 🔒 Optional — to re-enable later
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // 🔓 Auth temporarily bypassed — re-enable after everything is stable
  const isValid = true
  if (!isValid) {
    return res.status(401).json({ error: 'Unauthorized Make webhook' })
  }
  ...
  try {
    // 🧠 Generate structured reminder from AI
    ...
    // 🔥 Save to Firestore under correct child
